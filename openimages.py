...
def coreset_with_target_alignment(
    ...
    *,
    ...
    # score_order: str = "low",  # "high" or "low"
    fraction_pick_low_scores = 0.0, # in [0, 1]; only relevant if scores provided; 1.0 means pick lowest, 0.0 means pick highest
    ...
    if scores is not None:
        ...
        if scores_arr.ndim != 1 or scores_arr.shape[0] != labels_base.shape[0]:
            raise ValueError("scores must be a 1D array with the same length as labels_base")

        if not (0.0 <= fraction_pick_low_scores <= 1.0):
            raise ValueError("fraction_pick_low_scores must be in [0, 1]")

        scores_arr = scores_arr.astype(np.float64, copy=False)

        # Put NaNs in the "middle" so they don't get preferentially chosen
        med = np.nanmedian(scores_arr)
        if not np.isfinite(med):
            med = 0.0
        scores_arr = np.where(np.isnan(scores_arr), med, scores_arr)
    ...
    for c in range(num_classes):
        ...
        if scores_arr is None:
            ...
            cls_scores = scores_arr[cls]
            jitter = rng.random(cls_scores.shape) * 1e-12
            order = np.argsort(cls_scores + jitter, kind="mergesort")  # low scores first
            idxs_by_class[c] = cls[order]
    ...
    def _materialize_indices(counts: np.ndarray) -> np.ndarray:
        ...
        for c, take in enumerate(counts):
            ...
            if scores_arr is None:
                ...
                n_low = int(round(fraction_pick_low_scores * take))
                n_low = max(0, min(n_low, take))
                n_high = take - n_low
                ...
                if n_high == 0:
                    ...
                    pick = np.concatenate([cls[:n_low], cls[-n_high:]])
