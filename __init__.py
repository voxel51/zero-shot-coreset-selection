...
def compute_zcore_scores(sample_collection: SampleCollection) -> None:    
    return None

class ComputeZCoreScore(foo.Operator):
    @property
    def config(self):
        _config = foo.OperatorConfig(
            name="compute_zcore_score",
            label="Coreset: Compute ZCore scores",
            description="Compute ZCore scores for the dataset",
            dynamic=True,
        )
        _config.icon = "/assets/zcore.svg"
        return _config
    ...
    def execute(self, ctx):
        ...
        response = compute_zcore_scores(sample_collection)
        ctx.ops.reload_dataset()
        return response

    def resolve_output(self, ctx):
        outputs = types.Object()
        outputs.str("num_scores_computed", label="Number of ZCore scores computed")
        header = "ZCore Score Results"
        return types.Property(outputs, view=types.View(label=header))
...
def register(plugin):
    ...
    plugin.register(ComputeZCoreScore)
